...
void odom_estimation(){     // ! core
    while(1){
        if(!pointCloudEdgeBuf.empty() && !pointCloudSurfBuf.empty()){
            ...
            Eigen::Quaterniond q_current(odomEstimation.odom.rotation()); //q_current.normalize();
            Eigen::Vector3d t_current = odomEstimation.odom.translation();

            // 旋转追踪的目标轴 以y轴行进    // me add
            ...
            if (t_current(2)>0.3)
			    t_current(2) = 0.3;         // 地面上, z轴飘移限制, 根据实际地面平整度调试
            else if(t_current(2)<-0.15) { 
                t_current(2) = -0.15;
            }
            ...
            if (is_publish_tf) {    
                ...
                br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), parent_frame_id, tracked_frame_id));
            } 
            
            // publish odometry
            nav_msgs::Odometry laserOdometry; 
            laserOdometry.header.frame_id = parent_frame_id;
            laserOdometry.child_frame_id = tracked_frame_id;
...
int main(int argc, char **argv)
{
    ...
    nh.getParam("/odom_frame_id", parent_frame_id); 
    nh.getParam("/odom_child_frame_id", tracked_frame_id); 
    ...
    pubLaserOdometry = nh.advertise<nav_msgs::Odometry>("/odom", 10);      // 实时里程计
